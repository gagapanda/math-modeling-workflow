...
- Added a dedicated abstract-writing route and scaffolded evidence sheet. Final abstracts now start from frozen per-subproblem result IDs, display precision, validation/failure evidence, claim type and indispensable boundaries; the workflow adds abstract-only reconstruction, cross-position reconciliation, evidence-preserving compression order and final-page review without treating AI review as an independent judge or importing fixed scores, lengths or case-specific prose from local materials.
