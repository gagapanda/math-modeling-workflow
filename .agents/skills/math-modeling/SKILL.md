...
For the optional controlled 2026 CUMCM LaTeX path, start from `templates/cumcm-2026-latex/paper.tex` for the anonymous electronic paper. Use `paper-print.tex` only for the separate paper-print copy that includes the commitment and numbering pages. Preserve the enforced order `abstract -> main text -> AI declaration -> references -> appendices`, keep the table of contents disabled, compile with XeLaTeX, and repeat the complete page-region and visual review after every layout-sensitive change. The template is a maintained project asset derived from reviewed material; current official rules still override it.
