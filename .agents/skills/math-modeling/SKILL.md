...
- For DOCX paragraph rewrites, follow `references/paper-narrative-rewrite.md`: choose explicit per-segment style donors, preserve namespace references with `scripts/docx_rewrite_safety.py`, and require actual Word open/export evidence; XML checks alone are not compatibility or visual approval.
