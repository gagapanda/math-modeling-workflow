...
> 当前仓库处于公开版骨架阶段。内容正在从私人工作区筛选、脱敏和验证，尚未包含完整案例、课程资料或优秀论文。
...
当前是采用 MIT 的 source-only 公开候选。`scripts/check_release_inventory.py --require-release` 检查来源清单与许可声明，不代表完整法律审查、远程上传授权或比赛提交通过；实际上传前仍须核对待发布集合及隐私。
